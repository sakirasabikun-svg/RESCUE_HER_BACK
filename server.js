...
// MongoDB Schemas (গুগল সাইন-ইনের সুবিধার্থে googleId এবং photoURL স্কিমাতে যুক্ত করা হয়েছে)
...
// 🌐 AUTH API: NEW GOOGLE SIGN-IN / SIGN-UP
app.post('/api/google-login', async (req, res) => {
  try {
    const { email, name, googleId, photoURL } = req.body;

    // ১. চেক করা যে এই ইমেইল দিয়ে ডাটাবেজে অলরেডি কোনো ইউজার আছে কি না
    let user = await User.findOne({ email });

    if (!user) {
      // ২. ইউজার না থাকলে নতুন ইউজার তৈরি করা
      // সিকিউরিটির জন্য গুগল আইডি-কে হ্যাশ করে পাসওয়ার্ড ফিল্ডে ডামি ডাটা হিসেবে সেভ করা হলো
      ...
      user = await User.create({
        ...
        googleId,
        photoURL,
        phone: '', // খালি থাকবে, পরে প্রোফাইল থেকে ইউজার এটি আপডেট করতে পারবে
        blood_group: ''
      });
    }

    // ৩. ইউজারের জন্য সিকিউর JWT টোকেন জেনারেট করা
    const token = jwt.sign({ id: user._id, email: user.email }, JWT_SECRET, { expiresIn: '7d' });
